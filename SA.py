...
class SA(object):
    def __init__(self, num_city, data):
        ...
        # 显示初始化后的路径
        init_pathlen = self.compute_pathlen(self.fire, self.dis_mat)
        self.initial_length = init_pathlen
        init_best = self.location[self.fire]

        # 存储存储每个温度下的最终路径，画出收敛图
        self.iter_x = [0]
        self.iter_y = [init_pathlen]

        # 记录搜索过程
        self.search_paths = [self.fire.copy()]
        self.search_lengths = [init_pathlen]
        self.iter_time = []
    ...
    def sa(self):
        ...
        while self.T0 > self.Tend:
            ...
            # 记录搜索过程
            self.search_paths.append(self.fire.copy())
            self.search_lengths.append(best_length)
    ...
    def get_performance_metrics(self):
        ...
        return {
            ...
            'improvement_ratio': (self.initial_length - self.iter_y[-1]) / self.initial_length,
            'convergence_data': {
