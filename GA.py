...
class GA(object):
    def __init__(self, num_city, num_total, iteration, data):
        ...
        self.initial_length = 1. / scores[sort_index[0]]

        # 存储每个iteration的结果，画出收敛图
        self.iter_x = [0]
        self.iter_y = [1. / scores[sort_index[0]]]
        self.best_record = [1. / scores[sort_index[0]]]
        
        # 记录搜索过程
        self.search_paths = [self.fruits[sort_index[0]].copy()]
        self.search_lengths = [1. / scores[sort_index[0]]]
        self.iter_time = []
    ...
    def run(self):
        ...
        for i in range(1, self.iteration + 1):
            ...
            # 记录搜索过程
            self.search_paths.append(tmp_best_one.copy())
            self.search_lengths.append(1. / tmp_best_score)
    ...
    def get_performance_metrics(self):
        """返回性能指标"""
        return {
            'best_length': 1. / self.best_record[-1],
            'execution_time': self.total_time,
            'improvements': len(self.best_record),
            'best_iteration': np.argmin(self.best_record),
            'initial_length': self.initial_length,
            'improvement_ratio': (self.initial_length - 1./self.best_record[-1]) / self.initial_length,
            'convergence_data': {
                'iterations': self.iter_x,
                'lengths': self.best_record,
